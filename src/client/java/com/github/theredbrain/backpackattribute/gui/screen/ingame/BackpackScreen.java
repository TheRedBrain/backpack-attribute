...
public class BackpackScreen extends AbstractContainerScreen<BackpackMenu> {
    ...
    public boolean keyPressed(final KeyEvent event) {
        if (KeyBindingsRegistry.openBackpackScreen.matches(event)) {
            this.onClose();
            return true;
        }
...
//        this.renderBackground(graphics, mouseX, mouseY, a);
    ...
    public void extractBackground(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final float a) {
        ...
        graphics.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND_TEXTURE, i, j, 0, 0, this.imageWidth, this.imageHeight, this.imageWidth, this.imageHeight);

        for (k = 0; k < (BackpackAttributeClient.CLIENT_CONFIG.show_inactive_backpack_slots.get() ? 27 : Math.min(this.backpackCapacity, 27)); ++k) {
            m = (k / 9);
            graphics.blit(RenderPipelines.GUI_TEXTURED, SLOT_TEXTURE, i + 7 + (k - (m * 9)) * 18, j + 17 + (m * 18), 0, 0, 18, 18, 18, 18);
        }
        for (k = 0; k < (showInactiveInventorySlots ? 27 : Math.min(this.inventorySize, 27)); ++k) {
            m = (k / 9);
            graphics.blit(RenderPipelines.GUI_TEXTURED, SLOT_TEXTURE, i + 7 + (k - (m * 9)) * 18, j + 83 + (m * 18), 0, 0, 18, 18, 18, 18);
        }
        for (k = 0; k < (showInactiveInventorySlots ? 9 : Math.min(this.hotbarSize, 9)); ++k) {
            graphics.blit(RenderPipelines.GUI_TEXTURED, SLOT_TEXTURE, i + 7 + k * 18, j + 141, 0, 0, 18, 18, 18, 18);
        }
    }
}
